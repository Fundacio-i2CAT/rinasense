...
BaseType_t xEnrollmentHandleStartR(string_t xRemoteApName, serObjValue_t *pxSerObjValue)
{
        ...
        pxEnrollmentMsg = pxSerdesMsgEnrollmentDecode((uint8_t *)pxSerObjValue->, pxSerObjValue->xMessageLength);
...
BaseType_t xEnrollmentHandleCreate(struct ribObject_t *pxRibObject, serObjectValue_t *pxObjValue, string_t xRemoteProcessName,
                                   string_t xLocalProcessName, int invokeId, portId_t xN1Port)
{
        /*Decode ObjectValue*/
        neighborMessage_t *pxNeigborMsg;

        pxNeigborMsg = pxserdesMsgDecodeNeighbor((uint8_t *)pxObjValue->pvSerBuffer, pxObjValue->xSerLength);
